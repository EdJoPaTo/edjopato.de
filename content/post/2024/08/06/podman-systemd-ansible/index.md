...
Und `podman-auto-update` ist schon ein nettes Konzept.
...
Podman unterstützt auch schon länger Kubernetes YAML Dateien.
...
Da zum Beispiel `--init` kein Konzept in Kubernetes ist, muss es eher umständlich mit Annotationen abgebildet werden.
Wenn ich auch hier wieder dabei bin, Podman CLI Argumente nachzudengeln, kann ich das auch direkt machen.

Podman hat auch einen Befehl um systemd Service Dateien zu generieren.
...
Mittlerweile ist der Befehl abgekündigt und "Quadlet" empfohlen.
...
Podman hat `--rm` als Argument, um den Container, sobald dieser beendet wurde, zu löschen.
Da die Logs im journald stecken, brauche ich den Container an sich nicht mehr.
Nicht mehr laufende Container mittels `podman container ls --all` sehen zu können, kann aber auch nett sein.
Und da `--replace` bestehende Container ersetzen kann, warum nicht da lassen?
...
(Sowohl `containers.podman.podman_network` als auch `--network` funktionieren ohne irgendwas in meiner Ansible Rolle, daher ist dies nicht Teil davon.)
...
Ich habe mir angewöhnt für persistente Daten immer Bindings anzulegen.
...
Mit Podman und `--tmpfs` geht das ganze noch etwas entspannter.
...
Bei `podman run --env=secret=123` wär das unpraktisch.
...
### `systemctl --user` oder `User=`

Ein Teil des Unterfangens war ja auch von den `root` Prozessen wegzukommen.
Erster Ansatz war `User=` in System-Services.
Dann läuft Podman selbst als User, also muss auch `podman-auto-update` als User laufen und kann dann die System-Services nicht neu starten.
Also alles als User.

### Webserver

Einige Dienste stellen eine Weboberfläche bereit, die ich gerne nutzen würde.
Dafür gebe ich beim Container `--publish=[::1]:hostport:containerport` an und konfiguriere dies in einem `reverse_proxy localhost:hostport` in einer eigenen Caddyfile in `/etc/Caddyfile.d/`, die ebenfalls von Ansible erstellt wird.
